export const ContactLang = {
	ru: {
		considering: {
			title: 'Рассматриваю варианты удалённой работы',
			button: 'Найми меня',
		},
		form: {
			title: 'Свяжитесь со мной',
			name: 'Имя',
			email: 'Эл. адрес',
			message: 'Сообщение',
			button: 'Отправить сообщение',
			validate: {
				required: 'Обязательное поле',
				lessName: 'Должно быть не более 50 символов.',
				lessMessage: 'Должно быть не более 500.',
				invalidEmail: 'Неверный адрес электронной почты',
			},
			success: 'Письмо успешно было отправлено 😊',
		},
	},
	en: {
		considering: {
			title: 'CONSIDERING REMOTE WORK OPTIONS',
			button: 'Hire me',
		},
		form: {
			title: 'Contact With Me',
			name: 'name',
			email: 'email',
			message: 'message',
			button: 'Send message',
			validate: {
				required: 'Required',
				lessName: 'Must be 15 characters or less',
				lessMessage: 'Must be 15 characters or less',
				invalidEmail: 'Invalid email address',
			},
			success: 'Email sent successfully 😊',
		},
	},
	ukr: {
		considering: {
			title: 'Розглядаю варіанти віддаленої роботи',
			button: 'Найми мене',
		},
		form: {
			title: "Зв'яжіться зі мною",
			name: "Ім'я",
			email: 'Ел. адрес',
			message: 'Повідомлення',
			button: 'Надіслати повідомлення',
			validate: {
				required: "Обов'язкове поле",
				lessName: 'Має бути не більше 50 символів.',
				lessMessage: 'Має бути не більше 500.',
				invalidEmail: 'Неправильна адреса електронної пошти',
			},
			success: 'Лист успішно був надісланий 😊',
		},
	},
	ro: {
		considering: {
			title: 'OPȚIUNILE DE LUCRU DE LA DISTANȚĂ',
			button: 'Angajeaza-ma',
		},
		form: {
			title: 'Contactați cu Mine',
			name: 'Nume',
			email: 'E-mail',
			message: 'Mesaj',
			button: 'Trimite mesaj',
			validate: {
				required: 'Necesar',
				lessName: 'Trebuie să aibă 15 caractere sau mai puțin',
				lessMessage: 'Trebuie să aibă 15 caractere sau mai puțin',
				invalidEmail: 'Adresa email invalida',
			},
			success: 'Email-ul a fost trimis cu succes 😊',
		},
	},
};
